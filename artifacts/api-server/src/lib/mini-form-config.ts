...
export const MINI_FORM_CONFIGS: Record<string, MiniFormConfig> = {
  ...
  freight: {
    ...
    fields: [
      ...
      { name: "shipment_type",        label: "Jenis Pengiriman",     type: "select", required: true,  options: ["FCL", "LCL", "Air Freight", "Darat"], helpText: "FCL / LCL / Air Freight / Darat" },
  ...
  "field-booking": {
    ...
    fields: [
      ...
      { name: "field_type",     label: "Jenis Fasilitas",       type: "select",   required: true,  options: ["Lapangan Badminton A", "Lapangan Badminton B", "Lapangan Tenis", "Lapangan Multi Guna", "GYM", "Meja Billiard"] },
      { name: "booking_date",   label: "Tanggal Main",          type: "date",     required: true },
      { name: "people_count",  label: "Jumlah Orang",          type: "number",   required: false, placeholder: "1" },
      ...
      { name: "payment_method", label: "Metode Pembayaran",     type: "select",   required: true,  options: ["Transfer Bank", "QRIS"] },
